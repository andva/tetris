...
void Ai::CalculateOptimal()
{
	std::shared_ptr<Piece> optimalP;

	float optimal = -999999999.0f;
	int32_t pcStore = 0;
	Tetromino ht = mPieceManager.HasHoldType() ? mPieceManager.GetHoldType() : mPieceManager.GetNextType();
	for (int pc = 0; pc < 2; pc++) {
		for (int i = 0; i < 4; i++) {
			for (int x = 0; x < 9; x++) {
				std::shared_ptr<Piece> p = PieceFactory::CreatePiece(pc == 0 ? ht : mPieceManager.GetPiece()->GetType());
				mPieceManager.ReplaceCurrentPiece(p);
				for (int r = 0; r < i; ++r) {
				...
				std::shared_ptr<Grid> gCopy(new Grid(mGame.GetGrid()));
