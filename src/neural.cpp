...
void Genome::GenerateNetwork()
{
	...
	for (int i = 0; i < 10 * 20; i++) {
		mNetwork.mNeurons[i] = Neuron();
	}

	// Create output neurons for buttons
	for (int o = 0; o < NUM_OUTPUTS; o++) {
		mNetwork.mNeurons[mMaxNeuron + o] = Neuron();
	}
	...
	for (auto gene : mGenes) {
		if (gene->mEnabled) {
			if (mNetwork.mNeurons.find(gene->mOut) == mNetwork.mNeurons.end()) {
				mNetwork.mNeurons[gene->mOut] = Neuron();
			}
			mNetwork.mNeurons[gene->mOut].mIncoming.push_back(gene);

			if (mNetwork.mNeurons.find(gene->mInto) == mNetwork.mNeurons.end()) {
				mNetwork.mNeurons[gene->mInto] = Neuron();
			}
		}
	}
}
...
void Genome::EvaluateNetwork(const GridData& input)
{
	...
	for (int i = 0; i < 10 * 20; i++) {
		...
		mNetwork.mNeurons[i].mValue = static_cast<float>(input[y][x]);
	}
	for (auto& neuron : mNetwork.mNeurons) {
		float sum = 0;
		for (int i = 0; i < neuron.second.mIncoming.size(); i++) {
			auto incoming = neuron.second.mIncoming[i];
			auto other_incoming = mNetwork.mNeurons[incoming->mInto];
	...
	if (mNetwork.mNeurons[MAX_NODES + MOVE_LEFT].mValue > 0) {
		mGame.ExecuteAction(Dir::LEFT, Action::MOVE);
	}
	else if (mNetwork.mNeurons[MAX_NODES + MOVE_RIGHT].mValue > 0) {
		mGame.ExecuteAction(Dir::RIGHT, Action::MOVE);
	}
	else if (mNetwork.mNeurons[MAX_NODES + MOVE_DOWN].mValue > 0) {
		mGame.ExecuteAction(Dir::DOWN, Action::MOVE);
	}
	else if (mNetwork.mNeurons[MAX_NODES + ROTATE_LEFT].mValue > 0) {
		mGame.ExecuteAction(Dir::LEFT, Action::ROTATE);
	}
	else if (mNetwork.mNeurons[MAX_NODES + ROTATE_RIGHT].mValue > 0) {
		mGame.ExecuteAction(Dir::RIGHT, Action::ROTATE);
	}
	else if (mNetwork.mNeurons[MAX_NODES + HOLD].mValue > 0) {
		mGame.Hold();
	}
	else if (mNetwork.mNeurons[MAX_NODES + DROP].mValue > 0) {
		mGame.Drop();
	}
}
...
Genome Crossover(const Genome& g1, const Genome& g2) {
	...
	for (std::shared_ptr<Gene> gene : g1.mGenes) {
		...
		if (gene2 != newInnovations.end() && gene2->second->mEnabled && RandomEngine::GetRandomValue(0, 1) == 1) {
			child.mGenes.push_back(std::make_shared<Gene>(*gene2));
		}
		else {
